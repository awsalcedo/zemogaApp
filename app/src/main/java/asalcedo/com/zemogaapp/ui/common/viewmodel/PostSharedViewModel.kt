...
class PostSharedViewModel @Inject constructor(
    ...
    private val _allUsers = MutableLiveData<List<UserItem>>()
    val allUsers: LiveData<List<UserItem>> get() = _allUsers
    ...
    fun onCreate() {
        viewModelScope.launch {
            ...
            // Call the usescase
            val result = getPostUseCase.getAllPost()
            val users = getUserUseCase.getAllUsers()
            if (!result.isNullOrEmpty() && !users.isNullOrEmpty()) {
    ...
    fun setInfoLiveDataDeleteAllPost() {
        _postModel.postValue(mutableListOf())
    }
}